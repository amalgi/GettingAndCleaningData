...
2.Download the file. Put it in the Data folder. 
...
8.Read the features.txt file. This tells which variables in dt are measurements for the mean and standard deviation.

dtFeatures <- fread(file.path(pathIn, "features.txt"))
setnames(dtFeatures, names(dtFeatures), c("featureNum", "featureName"))

9.Subset only measurements for the mean and standard deviation.
dtFeatures <- dtFeatures[grepl("mean\\(\\)|std\\(\\)", featureName)]

10.Convert the column numbers to a vector of variable names matching columns in dt.
...
12.Seperate features from featureName using the helper function grepthis.
